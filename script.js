    ...
    function updateSitecheckerMessage() {
        setTimeout(() => {
            ...
            if (officialUrls.includes(pageUrl)) {
                console.log("DEBUG: Official Fenixo URL detected.");
                iconElement.classList.add("fa", "fa-circle-check", "sitechecker-secure");
                messageElement.innerHTML = `This link <b>(${pageUrl})</b> is a secure, official Fenixo Browser link.`;
            } else {
                console.log("DEBUG: UNOFFICIAL URL detected!");
                iconElement.classList.add("fa", "fa-triangle-exclamation", "sitechecker-warning");
                messageElement.innerHTML = `This link <b>(${pageUrl})</b> is not an official Fenixo Browser link. Use at your own risk, or find a list of official Fenixo Browser links <a href="https://github.com/dinguschan-owo/Fenixo/blob/main/README.md" target="_blank" class="sitechecker-link">here</a>.`;
            }
...
(function () {
    document.addEventListener("DOMContentLoaded", function () {
        console.log("Logging: Fenixo started");
        displayFenixoStarted();
    ...
    function displayFenixoStarted() {
        let logContainer = document.getElementById("logContainer");

        if (!logContainer) {
            setTimeout(displayFenixoStarted, 100);
            return;
        }

        addLogEntry(`[Fenixo Log] Fenixo started successfully`, true);
    }
    ...
    function logEvent(message) {
        ...
        addLogEntry(`[Fenixo Log] ${message}`);
    }
...
const FenixoMessageHistory = [];
const Fenixo_API_KEY_PARTS = [
...
function getFenixoApiKey() {
  const filteredParts = Fenixo_API_KEY_PARTS.filter(part => part !== 'X' && uselessChars.includes(part));
  return filteredParts.join('');
}


const FenixoSystemMessage = {
  role: "system",
  content: `You are Fenixo AI, an advanced AI assistant designed to be helpful, knowledgeable, and adaptable. You were made by dinguschan.`
};
...
chatbotToggler.addEventListener("click", () => {
  ...
  if (!existingWelcomeMessage) {
    addFenixoMessage("Hi there! How may I assist you?", false);
  }
});
...
sendChatBtn.addEventListener("click", sendFenixoMessage);
...
async function sendFenixoMessage() {
  const userMessage = chatInput.value.trim();
  if (!userMessage) return;

  addFenixoMessage(userMessage, true);
  chatInput.value = '';

  FenixoMessageHistory.push({ role: "user", content: userMessage });
  const loadingElement = addLoadingMessage();

  try {
    const response = await tryFenixoModels(userMessage);
    ...
    FenixoMessageHistory.push({ role: "assistant", content: formattedResponse });
    loadingElement.remove();
    addFenixoMessage(formattedResponse, false);
  } catch (error) {
    loadingElement.remove();
    addFenixoMessage(`Error: ${error.message}`, false);
  }
}

function addFenixoMessage(content, isUser) {
...
async function tryFenixoModels(userMessage) {
  const models = [
  ...
  for (let model of models) {
    try {
      const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${getFenixoApiKey()}`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          model: model.name,
          messages: [FenixoSystemMessage, ...FenixoMessageHistory],
          temperature: 0.7,
          max_tokens: 2048,
...
𝗦𝘂𝗴𝗴𝗲𝘀𝘁𝗲𝗱 𝗔𝗰𝘁𝗶𝗼𝗻𝘀:
• Visit [https://github.com/dinguschan-owo/Fenixo] and verify your browser is up to date.
...
• If all else fails, visit the above mentioned offical Github page [https://github.com/dinguschan-owo/Fenixo] and redownload the latest stable version (Fenixo v1.𝟽.0).
`;
...
const trustedSchemes = ['Fenixo://', 'https://', 'http://'];
...
function openGitHub() {
    window.open('https://github.com/dinguschan-owo/Fenixo/', '_blank');
}
...
function updateTabContent(url, content, tab) {
  ...
  <div class="dropdown-contentaa" id="engineDropdown-${currentTabIndex}">
    ...
    } else if (url === 'Fenixo://settings') {
        content.innerHTML = `
        <div class="sidebarvv">
  <h1>Fenixo Settings</h1>
...
<div class="content-containervv" id="miscvv">
  <h31>Customize Fenixo's Appearance</h31><div class="theme-preview-container">
<div class="theme-preview" onclick="activatePreview(this)">
  <div class="browser-simulation">
    ...
    <div class="browser-content"><div class="hetitle">Fenixo</div><div class="titlelinee"></div><div class="dropmed"></div><div class="send4"></div></div>
  </div>
...
<div class="theme-preview-lightmode" onclick="activatePreview(this)">
  <div class="browser-simulation-lightmode">
    ...
    <div class="browser-content-lightmode"><div class="hetitle-lightmode">Fenixo</div><div class="titlelinee-lightmode"></div><div class="dropmed-lightmode"></div><div class="send4-lightmode"></div></div>
  </div>
  <p class="theme-name-lightmode"><i>Dark Ember</i> by bromse</p>
        ...
        `;
        tab.querySelector('.tab-nameaa').textContent = 'Fenixo Settings';
        tabs[currentTabIndex].content = content.innerHTML;
        tabs[currentTabIndex].url = url;
    ...
    updateLockIcon(url);
    updateSpecialDivs(url);
    if (url === 'Fenixo://start' || url === 'Fenixo://settings' || url === 'Fenixo://urls') {
...
function updateSpecialDivs() {
    const checkURL = () => {
        ...
        const isFenixoURL = urlInput && urlInput.value.startsWith('Fenixo://');
        const officialC = document.querySelector('.official-caa');
        const official = document.querySelector('.officialaa');

        if (officialC) {
            officialC.style.opacity = isFenixoURL ? '1' : '0';
            officialC.style.pointerEvents = isFenixoURL ? '' : 'none';
        }
        if (official) {
            official.style.opacity = isFenixoURL ? '1' : '0';
            official.style.pointerEvents = isFenixoURL ? '' : 'none';
        }
    };
...
// Function to open Fenixo in an about:blank tab
...
// Function to open Fenixo in a blob: tab
...
document.querySelectorAll('.tabaa').forEach((tab, index) => {
    const url = tab.querySelector('.tab-nameaa').textContent === 'New Tab' ? 'Fenixo://start' : document.getElementById('url-baraa').value;
...
function handleSquareButtonClick() {
    ...
    // Reset URL bar
    document.getElementById('url-baraa').value = 'Fenixo://start';

    // Reset tab content
    updateTabContent('Fenixo://start', firstTabContent, firstTab);

    // Reset tab history
    const history = getTabHistory(0);
    history.history = ['Fenixo://start'];
...
function showUrlsList(content, tab) {
    content.innerHTML = `
        <h2 style="font-size: 48px; margin-bottom: 20px; border-bottom: 3px solid #fff;">Fenixo Internal URLs</h2>
        <ul style="list-style-type: none; padding: 0;">
            <li style="margin-bottom: 15px;">
                <a href="javascript:void(0);" data-url="Fenixo://start" class="custom-url-link" style="font-size: 24px; color: #66ffa1; text-decoration: none;">Fenixo://start&nbsp;<i class="fa-solid fa-link" style="font-size: 18px; color: #66ffa1;"></i></a>
                <p style="margin: 5px 0 0 0; color: #b3b3b3;">Fenixo start page</p>
            </li>
            <li style="margin-bottom: 15px;">
                <a href="javascript:void(0);" data-url="Fenixo://settings" class="custom-url-link" style="font-size: 24px; color: #66ffa1; text-decoration: none;">Fenixo://settings&nbsp;<i class="fa-solid fa-link" style="font-size: 18px; color: #66ffa1;"></i></a>
                <p style="margin: 5px 0 0 0; color: #b3b3b3;">Fenixo settings page</p>
            </li>
            <li style="margin-bottom: 15px;">
                <a href="javascript:void(0);" data-url="Fenixo://urls" class="custom-url-link" style="font-size: 24px; color: #66ffa1; text-decoration: none;">Fenixo://urls&nbsp;<i class="fa-solid fa-link" style="font-size: 18px; color: #66ffa1;"></i></a>
                <p style="margin: 5px 0 0 0; color: #b3b3b3;">List of Fenixo internal URLs</p>
            </li>
        </ul>
    `;
    tab.querySelector('.tab-nameaa').textContent = 'Fenixo URLs';
...
function downloadPage() {
    ...
    const url = "https://raw.githubusercontent.com/dinguschan-owo/Fenixo/refs/heads/main/Offline-File/Fenixo-Offline.html";
    fetch(url)
        .then(response => response.blob()).then(blob => {
            const a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            a.download = "Fenixo-Offline.html";
            a.click();
        })
        .catch(error => {
    ...
    function fetchLocalFile() {
        fetch('/Offline-File/Fenixo-Offline.html')
            .then(response => {
                if (!response.ok) {
